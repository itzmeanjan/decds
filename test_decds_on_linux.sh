...
cargo run --profile optimized -- repair -c broken -o repaired-with-16
echo "$(sha256sum random.data | awk '{print $1}') repaired-with-16/repaired.data" | sha256sum --check
...
cargo run --profile optimized -- repair -c broken -o repaired-with-15
echo "$(sha256sum random.data | awk '{print $1}') repaired-with-15/repaired.data" | sha256sum --check
...
cargo run --profile optimized -- repair -c broken -o repaired-with-14
echo "$(sha256sum random.data | awk '{print $1}') repaired-with-14/repaired.data" | sha256sum --check
...
cargo run --profile optimized -- repair -c broken -o repaired-with-13
echo "$(sha256sum random.data | awk '{print $1}') repaired-with-13/repaired.data" | sha256sum --check
...
cargo run --profile optimized -- repair -c broken -o repaired-with-12
echo "$(sha256sum random.data | awk '{print $1}') repaired-with-12/repaired.data" | sha256sum --check
...
cargo run --profile optimized -- repair -c broken -o repaired-with-11
echo "$(sha256sum random.data | awk '{print $1}') repaired-with-11/repaired.data" | sha256sum --check
...
cargo run --profile optimized -- repair -c broken -o repaired-with-9 | tee console.out; test ${PIPESTATUS[0]} -eq 1

# Clean up
rm -rf random.data broken repaired* console.out
